...
async function getRecipes() {
  ...
  return new Promise(getRecipesPromiseCallback);
  /**************************/
  // A4-A11 will all be *inside* the callback function we passed to the Promise
  // we're returning
  async function getRecipesPromiseCallback(resolve, reject) {
    ...
    for (let i = 0; i < RECIPE_URLS.length; i++){
      ...
      try {
        ...
        let recipeData = await fetch(recipeURL);
        ...
        //            "await" again
        let currentRecipe = await recipeData.json();
